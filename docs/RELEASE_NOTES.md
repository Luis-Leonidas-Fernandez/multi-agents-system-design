...
- Construcción del chain del supervisor en `features/supervisor/application/supervisor_chain.py`.
- División fina del supervisor en `features/supervisor/application/supervisor_chain.py`, `supervisor_routing.py` y `supervisor_shortcuts.py`.
- Centralización de factories ReAct en `application/services/agents_factory.py` con `_build_specialized_agent`.
- Reorganización de tools y adaptadores por feature en `features/*/infrastructure/`.
- Extracción de helpers de precio a `features/price/application/price_flow_helpers.py`.
- Extracción de helpers de seguridad a `features/security/application/api.py`.
- Extracción de helpers de audit a `core/helpers/audit_flow_helpers.py`.
- Extracción de helpers de persistencia a `core/helpers/persistence_flow_helpers.py`.
- Extracción de helpers de scraping a `core/helpers/scraping_flow_helpers.py`.
- Extracción de helpers de config a `core/helpers/config_flow_helpers.py`.
...
- Truncado compartido en `core/helpers/text_truncation.py`.
