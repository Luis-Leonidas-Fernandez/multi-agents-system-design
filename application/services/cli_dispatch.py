...
def _dispatch_help(_: list[str], __: str, ___, ____):
    return CLICommandResult(True, format_inspection_help())


def _dispatch_commands(_: list[str], __: str, ___, ____):
    return CLICommandResult(True, format_command_registry())


def _dispatch_command(parts: list[str], _: str, __, ___):
    if len(parts) < 2:
...
def _dispatch_context(parts: list[str], __: str, lifecycle, ___):
...
def _dispatch_inspect(_: list[str], __: str, lifecycle, ___):
...
def _dispatch_tasks(_: list[str], __: str, lifecycle, ___):
    summary = lifecycle.background_task_summary()
...
def _dispatch_task(parts: list[str], _: str, lifecycle, ___):
    if len(parts) < 2:
...
def _dispatch_cancel(parts: list[str], _: str, lifecycle, ___):
    if len(parts) < 2:
...
def _dispatch_retryable(_: list[str], __: str, lifecycle, ___):
...
def _dispatch_artifact(_: list[str], __: str, lifecycle, ___):
    artifact = lifecycle.export_artifact()
...
def _dispatch_bookmarks(_: list[str], __: str, lifecycle, ___):
    bookmarks = lifecycle.list_bookmarks()
    return CLICommandResult(True, format_bookmark_list(bookmarks))


def _dispatch_bookmark(parts: list[str], command: str, lifecycle, ___):
...
def _dispatch_checkpoint(parts: list[str], _: str, lifecycle, ___):
    if len(parts) < 2:
...
def _dispatch_replay(parts: list[str], _: str, lifecycle, runtime):
...
def _dispatch_memory(parts: list[str], _: str, ___, runtime):
    if len(parts) < 2:
...
def _dispatch_tools(_: list[str], __: str, ___, runtime):
    return CLICommandResult(True, format_tool_catalog(runtime.tool_catalog()))


def _dispatch_tool(parts: list[str], _: str, ___, runtime):
    if len(parts) < 2:
...
def _dispatch_impact(parts: list[str], _: str, ___, runtime):
    if len(parts) < 2:
...
def _dispatch_workers(_: list[str], __: str, lifecycle, runtime):
    workers = runtime.list_workers(lifecycle.session_id)
    return CLICommandResult(True, format_worker_list(workers))


def _dispatch_worker(parts: list[str], _: str, lifecycle, runtime):
    if len(parts) < 2:
...
def _dispatch_mailbox(_: list[str], __: str, lifecycle, runtime):
...
COMMAND_HANDLERS = {
    ...
    "state": _dispatch_context,
    "inspect": _dispatch_inspect,
    "status": _dispatch_inspect,
    "tasks": _dispatch_tasks,
    "task": _dispatch_task,
    "cancel": _dispatch_cancel,
    "retryable": _dispatch_retryable,
    "artifact": _dispatch_artifact,
    "bookmarks": _dispatch_bookmarks,
    "checkpoints": _dispatch_bookmarks,
    "bookmark": _dispatch_bookmark,
    "checkpoint": _dispatch_checkpoint,
    "prompts": _dispatch_prompts,
    ...
    "workers": _dispatch_workers,
    "worker": _dispatch_worker,
    "mailbox": _dispatch_mailbox,
}
...
def dispatch_inspection_command(user_input: str, lifecycle, runtime) -> CLICommandResult:
    ...
    if handler is not None:
        return handler(parts, command, lifecycle, runtime)
