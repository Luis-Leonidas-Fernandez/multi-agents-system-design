...
def load_memory_context(session_id: str) -> str:
    ...
    memory_file = Path("data") / "sessions" / session_id / "MEMORY.md"
...
async def distill_memory(state: dict, session_id: str) -> bool:
    """Destila los mensajes de la sesión en bullets y los guarda en data/sessions/{id}/MEMORY.md.
    ...
    try:
        ...
        memory_dir = Path("data") / "sessions" / session_id
