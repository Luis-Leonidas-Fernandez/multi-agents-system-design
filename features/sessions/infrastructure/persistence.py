...
Backend principal: SQLite (data/sessions/sessions.db).
Backend fallback:  JSONL (data/sessions/{session_id}.jsonl) si USE_SQLITE=false.
...
def load_messages(session_id: str) -> list[BaseMessage]:
    ...
    """
    if not _USE_SQLITE:
        return _load_jsonl(session_id, _SESSIONS_DIR)
...
def save_session(session_id: str, messages: list[BaseMessage]) -> None:
    ...
    """
    if not _USE_SQLITE:
        _save_jsonl(session_id, messages, _SESSIONS_DIR)
...
def _maybe_migrate_jsonl(session_id: str) -> None:
    ...
    msgs = _load_jsonl(session_id, _SESSIONS_DIR)
    if not msgs:
        _migrated.add(session_id)
        return
