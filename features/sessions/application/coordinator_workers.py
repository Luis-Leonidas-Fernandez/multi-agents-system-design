...
from application.services.coordinator_probe_orchestrator import (
    CoordinatorProbeOrchestrator,
    _extract_url_query,
    _search_web_query,
)
...
class CoordinatorWorkerService:
    ...
    def synthesize_probe_results(
        ...
        return {"response": response, "best_source": best["source_name"], "probe_results": ranked}


class CoordinatorRuntimeService(CoordinatorProbeOrchestrator):
    def _search_web_query(self, query: str) -> str:
        return _search_web_query(query)

    async def _extract_url_query(self, url: str):
...
coordinator_runtime_service = CoordinatorRuntimeService(coordinator_worker_service)
