...
Uso:
    python features/analytics/infrastructure/analytics.py [audit.jsonl]
    python features/analytics/infrastructure/analytics.py --no-show          # solo imprime, sin ventana
    python features/analytics/infrastructure/analytics.py --train            # entrena logistic regression
    python features/analytics/infrastructure/analytics.py --train --no-show  # entrena + guarda PNG
"""
import json
import os
import sys
...
def _resolve_path() -> str:
    ...
    if args:
        return args[0]
    return os.getenv("AGENTDOG_AUDIT_LOG", "./data/logs/agentdog_audit.jsonl")
...
def strategy_ranking(records: list) -> list:
    ...
    for (category, strategy), recs in sorted(groups.items()):
        ...
        # Penalizaciones normalizadas [0, 1] — mismos factores que la capa de orquestación
...
def export_policy_config(dominance: dict, path: Path) -> None:
    """Exporta policy.json versionado para que la capa de orquestación lo cargue en runtime.
