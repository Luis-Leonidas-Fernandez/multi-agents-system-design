...
def _isolate_persistence(tmp_path, monkeypatch):
    """
    Aisla el módulo features.sessions.infrastructure.persistence para cada test:
    ...
    """
    import features.sessions.infrastructure.persistence as persistence
...
def test_session_persistence_wrapper_delega_en_infra(monkeypatch):
    from features.sessions.application.session_persistence import SessionPersistence

    calls = []

    monkeypatch.setattr("features.sessions.infrastructure.persistence.list_sessions", lambda: ["a"])
    monkeypatch.setattr("features.sessions.infrastructure.persistence.load_messages", lambda session_id: [HumanMessage(content=session_id)])
    monkeypatch.setattr("features.sessions.infrastructure.persistence.save_message", lambda *args, **kwargs: calls.append((args, kwargs)))
    monkeypatch.setattr("features.sessions.infrastructure.persistence.save_session", lambda *args, **kwargs: calls.append((args, kwargs)))
...
def test_session_memory_wrapper_delega_en_infra(monkeypatch):
    from features.sessions.application.session_memory import SessionMemory

    calls = []

    monkeypatch.setattr("features.sessions.infrastructure.memory.load_memory_context", lambda session_id: f"mem:{session_id}")

    async def fake_distill(state, session_id):
        calls.append((state, session_id))
        return True

    monkeypatch.setattr("features.sessions.infrastructure.memory.distill_memory", fake_distill)
...
def test_row_to_msg_human():
    from features.sessions.infrastructure.persistence import _row_to_msg
